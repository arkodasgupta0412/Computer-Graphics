...
QPointF MainWindow::getIntersectionWA(const QPointF& p1, const QPointF& p2,
...
/*
QList<QList<QPointF>> MainWindow::weilerAthertonClip(const QList<QPointF>& polygon, const QRect& clipRect)
{
    struct Vtx { QPointF pos; bool isInter=false; bool isStart=false; int link=-1; bool visited=false; };

    QList<QList<QPointF>> resultPolygons;
    if (polygon.size() < 3) return resultPolygons;

    auto insideWin = [&](const QPointF& p)->bool {
        return p.x() >= clipRect.left() && p.x() <= clipRect.right() &&
               p.y() >= clipRect.top()  && p.y() <= clipRect.bottom();
    };

    auto equalPt = [&](const QPointF& a, const QPointF& b)->bool {
        return std::abs(a.x()-b.x()) < 1e-6 && std::abs(a.y()-b.y()) < 1e-6;
    };

    auto getInter = [&](const QPointF& p1, const QPointF& p2, const QPointF& q1, const QPointF& q2)->QPointF {
        return computeLineIntersection(p1, p2, q1, q2);
    };

    QList<Vtx> subj;  for (const QPointF& p : polygon) subj.append({p});
    QList<QPointF> winPoly;
    winPoly << QPointF(clipRect.left(),  clipRect.top())
            << QPointF(clipRect.right(), clipRect.top())
            << QPointF(clipRect.right(), clipRect.bottom())
            << QPointF(clipRect.left(),  clipRect.bottom());
    QList<Vtx> clip; for (const QPointF& p : winPoly) clip.append({p});

    bool allInside = true;
    for (const Vtx& v : subj) if (!insideWin(v.pos)) { allInside = false; break; }

    QList<QPair<int,Vtx>> subjIns, clipIns;

    for (int i = 0; i < subj.size(); ++i) {
        QPointF p1 = subj[i].pos;
        QPointF p2 = subj[(i+1) % subj.size()].pos;

        for (int j = 0; j < clip.size(); ++j) {
            QPointF w1 = clip[j].pos;
            QPointF w2 = clip[(j+1) % clip.size()].pos;

            QPointF I = getInter(p1, p2, w1, w2);
            if (!I.isNull()) {
                bool isStart = insideWin(p1) && !insideWin(p2);
                subjIns.append({i+1, {I, true, isStart, -1, false}});
                clipIns.append({j+1, {I, true, isStart, -1, false}});
            }
        }
    }
    ...
    if (subjIns.isEmpty()) {
        if (allInside) {
            ...
            QPointF center((clipRect.left()+clipRect.right())/2.0,
                           (clipRect.top()+clipRect.bottom())/2.0);
            if (isPointInsidePolygon(center, polygon)) {
                QList<QPointF> win;
                win << QPointF(clipRect.left(),  clipRect.top())
                    << QPointF(clipRect.right(), clipRect.top())
                    << QPointF(clipRect.right(), clipRect.bottom())
                    << QPointF(clipRect.left(),  clipRect.bottom());
                resultPolygons.append(win);
            }
        }
        return resultPolygons;
    }

    std::sort(subjIns.begin(), subjIns.end(),
              [](const QPair<int,Vtx>& a, const QPair<int,Vtx>& b){ return a.first > b.first; });
    for (const auto& ins : subjIns) subj.insert(ins.first, ins.second);

    std::sort(clipIns.begin(), clipIns.end(),
              [](const QPair<int,Vtx>& a, const QPair<int,Vtx>& b){ return a.first > b.first; });
    for (const auto& ins : clipIns) clip.insert(ins.first, ins.second);

    for (int i = 0; i < subj.size(); ++i) {
        if (!subj[i].isInter) continue;
        for (int j = 0; j < clip.size(); ++j) {
            if (clip[j].isInter && equalPt(subj[i].pos, clip[j].pos)) {
                subj[i].link = j;
                clip[j].link = i;
                break;
            }
    ...
    for (int i = 0; i < subj.size(); ++i) {
        if (!(subj[i].isInter && subj[i].isStart) || subj[i].visited) continue;

        QList<QPointF> curPoly;
        int idx = i;
        bool onSubj = true;

        do {
            Vtx& v = onSubj ? subj[idx] : clip[idx];
            v.visited = true;
            curPoly.append(v.pos);

            if (v.isInter && v.link != -1) {
                onSubj = !onSubj;
                idx = v.link;
            }
            idx = (idx + 1) % (onSubj ? subj.size() : clip.size());
        } while (curPoly.size() < 2 || !equalPt(curPoly.first(), curPoly.back()));

        curPoly.removeLast();
        if (curPoly.size() >= 3) resultPolygons.append(curPoly);
    }
...
QList<Vertex*> MainWindow::createVertexList(const QList<QPointF>& polygon, bool)
{
    QList<Vertex*> vertexList;
    for (const QPointF& point : polygon) vertexList.append(new Vertex(point));
    for (int i = 0; i < vertexList.size(); i++) vertexList[i]->next = vertexList[(i + 1) % vertexList.size()];
    return vertexList;
}

void MainWindow::findIntersections(QList<Vertex*>& subjectList, QList<Vertex*>& clipList, const QRect& clipRect)
{
    Q_UNUSED(clipRect)
    for (Vertex* s = 0, *sn = 0, *c = 0, *cn = 0, *sv = 0, *cv = 0; ; )
    {
        for (int i = 0; i < subjectList.size(); ++i) {
            s = subjectList[i];
            sn = s->next;
            QPointF p1 = s->point, p2 = sn->point;
            for (int j = 0; j < clipList.size(); ++j) {
                c = clipList[j];
                cn = c->next;
                QPointF p3 = c->point, p4 = cn->point;
                QPointF inter = computeLineIntersection(p1, p2, p3, p4);
                if (inter.isNull()) continue;
                if (peq(inter, p1) || peq(inter, p2) || peq(inter, p3) || peq(inter, p4)) continue;
                if (hasPointOnEdge(s, sn, inter)) continue;
                if (hasPointOnEdge(c, cn, inter)) continue;
                sv = new Vertex(inter, true);
                cv = new Vertex(inter, true);
                sv->neighbor = cv;
                cv->neighbor = sv;
                insertVertexInEdge(s, sn, sv);
                insertVertexInEdge(c, cn, cv);
            }
        }
        break;
    }
...
/*
void MainWindow::insertVertexInEdge(Vertex* start, Vertex* end, Vertex* newVertex)
{
    Vertex* current = start;
    double newDist = distance(start->point, newVertex->point);

    while (current->next != end &&
           distance(start->point, current->next->point) < newDist) {
...
/*
void MainWindow::markEntryExitPoints(QList<Vertex*>& subjectList, QList<Vertex*>& clipList)
{
    if (subjectList.isEmpty() || clipList.isEmpty()) return;
    QList<QPointF> clipPoints = getPointsFromVertexList(clipList);
    Vertex* start = subjectList.first();
    Vertex* cur = start;
    do {
        if (cur->isIntersection) {
            Vertex* prev = nullptr;
            Vertex* it = start;
            do { if (it->next == cur) { prev = it; break; } it = it->next; } while (it && it != start);
            QPointF mid((prev->point.x() + cur->point.x()) * 0.5, (prev->point.y() + cur->point.y()) * 0.5);
            bool wasInside = isPointInsidePolygon(mid, clipPoints);
            cur->isEntry = !wasInside;
            cur->visited = false;
            if (cur->neighbor) cur->neighbor->visited = false;
        }
        cur = cur->next;
    } while (cur && cur != start);
}
...
/*
QList<QPointF> MainWindow::getPointsFromVertexList(const QList<Vertex*>& vertexList)
{
    QList<QPointF> points;
    if (vertexList.isEmpty()) return points;
...
Vertex* MainWindow::findPreviousVertex(const QList<Vertex*>& vertexList, Vertex* target)
{
    for (Vertex* vertex : vertexList) {
        if (vertex->next == target) {
            return vertex;
        }
    }
    return nullptr;
...
QList<QList<QPointF>> MainWindow::extractClippedPolygons(QList<Vertex*>& subjectList, QList<Vertex*>&)
{
    QList<QList<QPointF>> result;
    if (subjectList.isEmpty()) return result;

    Vertex* ringStart = subjectList.first();

    for (Vertex* seed = findUnvisitedEntry(ringStart); seed; seed = findUnvisitedEntry(ringStart)) {
        QList<QPointF> poly;
        Vertex* cur = seed;
        int guard = 0;
        do {
            if (++guard > 10000) break;
            poly.append(cur->point);
            cur->visited = true;
            if (cur->isIntersection && cur->neighbor) {
                cur = cur->neighbor;
                cur->visited = true;
            }
            cur = cur->next;
        ...
        if (poly.size() >= 3) result.append(poly);
    }
...
void MainWindow::cleanupVertexList(QList<Vertex*>& vertexList)
{
    QSet<Vertex*> deletedVertices;

    for (Vertex* vertex : vertexList) {
        Vertex* current = vertex;
        while (current && !deletedVertices.contains(current)) {
            deletedVertices.insert(current);
            Vertex* next = current->next;
            if (!vertexList.contains(current)) {
                delete current;
            }
            current = next;
        }
    ...
    for (Vertex* vertex : vertexList) {
        if (!deletedVertices.contains(vertex)) {
            delete vertex;
        }
    }
